...
values (MRV) combined with least constrained value (LCV) in addition to Arc 
Consistency (AC). Rather than working through the chessboard row by row, MRV 
will prioritize rows with minimum remaining safe spots at each step. 
Then, Arc Consistency will hypothetically place a Queen in each safe spot in the 
target row and check whether all remaining rows have at least one safe spot. 
If not, then that safe spot is longer safe and pruned off from the domain. LCV 
will order the safe columns based on the number of remaining safe spots after 
hypothetical Queen placement in each of them (in an ascending order). Combining 
both ordering heuristics allows stronger and quicker detection of failures and 
early backtracks. The Arc Consistency acts as an additional filter and shrinks 
down the search state space size (safe cols in each row). 
"""


class N_Queens_Solver_MRV_LCV_AC:
    """
    ...
    def visualize_step(self):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{self.n}-Queens (MRV + LCV + Arc Consistency)")
        ax.invert_yaxis()
    ...
    """
    This function runs the recursive N-Queens Solver with MRV + LCV + AC. 
    """
