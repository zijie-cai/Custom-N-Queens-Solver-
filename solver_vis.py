...
def N_Queens_Solver_Vis():
    ...
    title = widgets.Label(value="Custom N-Queens Solver", 
                          style={'font_weight': 'bold', 'font_size': '20px'})
    
    # User input size N for chessboard
    size = widgets.BoundedIntText(value=8, min=4, max=32, step=1, 
                                           description='Size of N:', 
                                           layout=widgets.Layout(width='200px'))
    
    # Dropdown list for algorithm options
    algorithm_opt = widgets.RadioButtons(options=['Backtracking'], 
                                     description='Algorithm:')
    
    # Dropdown list for ordering options
    ordering_opt = widgets.RadioButtons(options=['None', 'MRV', 'LCV', 
                                                 'MRV + LCV'], 
                                    description='Ordering:', 
                                    layout=widgets.Layout(margin='8px 0 0 0'))

    # Dropdown list for filtering options
    filtering_opt = widgets.RadioButtons(options=['None', 'Forward Checking', 
                                              'Arc Consistency'], 
                                     description='Filtering:')

    # Click button to run the N-Queens Solver
    solve_button = widgets.Button(description="Solve N-Queens", 
                                    layout=widgets.Layout(margin='8px 0 8px 0'))
    ...
    left_box = widgets.VBox([title, size, algorithm_opt, ordering_opt, 
                             filtering_opt, solve_button, left_output_area])
    ...
    img_output_area = widgets.Output(layout = \
                                     widgets.Layout(margin='0 0 0 -50px'))
    ...
    def click_solve_button(b):
        ...
        with left_output_area:
            ...
            # Baseline
            if algorithm == 'Backtracking' and ordering == 'None' \
                and filtering == 'None':
                solver_no_vis = N_Queens_Solver(img_output_area, 
                                              progress_bar=None, 
                                              progress_label=None, 
                                              vis=False, 
                                              num_img=0, 
                                              size=n)
            # MRV
            elif algorithm == 'Backtracking' and ordering == 'MRV' \
                and filtering == 'None':
                solver_no_vis = N_Queens_Solver_MRV(img_output_area, 
                                                  progress_bar=None, 
                                                  progress_label=None, 
                                                  vis=False, 
                                                  num_img=0, 
                                                  size=n)
            # LCV
            elif algorithm == 'Backtracking' and ordering == 'LCV' \
                and filtering == 'None':
                solver_no_vis = N_Queens_Solver_LCV(img_output_area, 
                                                  progress_bar=None, 
                                                  progress_label=None, 
                                                  vis=False, 
                                                  num_img=0, 
                                                  size=n)
            # MRV + LCV
            elif algorithm == 'Backtracking' and ordering == 'MRV + LCV' \
                and filtering == 'None':
                solver_no_vis = N_Queens_Solver_MRV_LCV(img_output_area, 
                                                      progress_bar=None, 
                                                      progress_label=None, 
                                                      vis=False, 
                                                      num_img=0, 
                                                      size=n)
            # Forward Checking
            elif algorithm == 'Backtracking' and ordering == 'None' \
                and filtering == 'Forward Checking':
                solver_no_vis = N_Queens_Solver_FC(img_output_area, 
                                                 progress_bar=None, 
                                                 progress_label=None, 
                                                 vis=False, 
                                                 num_img=0, 
            ...
            # Arc Consistency
                ...
                solver_no_vis = N_Queens_Solver_AC(img_output_area, 
                                                 progress_bar=None, 
                                                 progress_label=None, 
                                                 vis=False, 
                                                 num_img=0, 
            ...
            # MRV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_no_vis = N_Queens_Solver_MRV_FC(img_output_area, 
                                                     progress_bar=None, 
                                                     progress_label=None, 
                                                     vis=False, 
                                                     num_img=0, 
            ...
            # LCV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_no_vis = N_Queens_Solver_LCV_FC(img_output_area, 
                                                     progress_bar=None, 
                                                     progress_label=None, 
                                                     vis=False, 
                                                     num_img=0, 
            ...
            # MRV + LCV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_no_vis = N_Queens_Solver_MRV_LCV_FC(img_output_area, 
                                                         progress_bar=None, 
                                                         progress_label=None, 
                                                         vis=False, 
                                                         num_img=0, 
            ...
            # MRV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_no_vis = N_Queens_Solver_MRV_AC(img_output_area, 
                                                     progress_bar=None, 
                                                     progress_label=None, 
                                                     vis=False, 
                                                     num_img=0, 
            ...
            # LCV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_no_vis = N_Queens_Solver_LCV_AC(img_output_area, 
                                                     progress_bar=None, 
                                                     progress_label=None, 
                                                     vis=False, 
                                                     num_img=0, 
            ...
            # MRV + LCV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_no_vis = N_Queens_Solver_MRV_LCV_AC(img_output_area, 
                                                         progress_bar=None, 
                                                         progress_label=None, 
                                                         vis=False, 
                                                         num_img=0, 
            ...
            if solver_no_vis.positions:
                ...
                print(f"Total Queen Placements: "
                      f"{solver_no_vis.queen_placement}")
                print(f"Total Backtracking Steps: "
        ...
        with right_output_area:
            ...
            max_img = 20 if solver_no_vis.step_number > 20 \
                            else solver_no_vis.step_number
            
            # Progress bar for tracking visualization generation 
            progress_bar = widgets.IntProgress(value=0, min=0, max=max_img, 
            ...
            if algorithm == 'Backtracking' and ordering == 'None' \
                and filtering == 'None':
                solver_vis = N_Queens_Solver(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            ...
                                            size=n)
            # MRV
            elif algorithm == 'Backtracking' and \
                ordering == 'MRV' and filtering == 'None':
                solver_vis = N_Queens_Solver_MRV(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # LCV
                ...
                and filtering == 'None':
                solver_vis = N_Queens_Solver_LCV(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # MRV + LCV
                ...
                and filtering == 'None':
                solver_vis = N_Queens_Solver_MRV_LCV(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_vis = N_Queens_Solver_FC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_vis = N_Queens_Solver_AC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # MRV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_vis = N_Queens_Solver_MRV_FC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # LCV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_vis = N_Queens_Solver_LCV_FC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # MRV + LCV + Forward Checking
                ...
                and filtering == 'Forward Checking':
                solver_vis = N_Queens_Solver_MRV_LCV_FC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # MRV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_vis = N_Queens_Solver_MRV_AC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # LCV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_vis = N_Queens_Solver_LCV_AC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # MRV + LCV + Arc Consistency
                ...
                and filtering == 'Arc Consistency':
                solver_vis = N_Queens_Solver_MRV_LCV_AC(img_output_area, 
                                            progress_bar, 
                                            progress_label, 
                                            vis=True, 
                                            num_img=solver_no_vis.step_number, 
            ...
            # Define function to update slider value when prev_button is clicked
            def click_prev_button(b):
                if slider.value > 0:
                    slider.value -= 1
            
            # Define function to update slider value when next_button is clicked
            ...
            buttons_box = widgets.HBox([prev_button, 
                                next_button],
                                layout = widgets.Layout(margin='0 0 0 50px'))
        
            # Output slider's value as it is changed and update image step
            slider.observe(update_image, names='value')

            # Wrap slider into horizontal widget box
            slider_box = widgets.HBox([slider], 
                                layout = widgets.Layout(margin='10px 0 0 50px'))

            # Combine slider, prev, next button and img output vertically 
            right_box = widgets.VBox([buttons_box, slider_box, 
                                    img_output_area],
