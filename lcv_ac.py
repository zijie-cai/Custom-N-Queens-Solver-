...
value (LCV) in addition to Arc Consistency (AC). The Solver will work through
...
order). Arc Consistency will hypothetically place a Queen in each safe spot in 
the current row and check whether all remaining rows have at least one safe 
spot. If not, then that safe spot is longer safe and pruned off from the domain.
...
class N_Queens_Solver_LCV_AC:
    ...
    def visualize_step(self):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{self.n}-Queens (LCV + Arc Consistency)")
        ax.invert_yaxis()
