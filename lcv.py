...
value (LCV). The Solver will work through row by row and LCV will order the 
safe columns based on the number of remaining safe spots after hypothetical 
Queen placement in each of them (in ascending order). Note while it is tempting
to associate LCV with maximizing number of safe spots to order the columns in
descending order, after experimenting, sorting the safe columns in ascending 
order is more effective and intuitive with failing fast strategy. 
"""


class N_Queens_Solver_LCV:
    """
    ...
    def visualize_step(self):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{self.n}-Queens (LCV)")
        ax.invert_yaxis()
    ...
    """
    This function runs the recursive N-Queens Solver with LCV. 
    """
