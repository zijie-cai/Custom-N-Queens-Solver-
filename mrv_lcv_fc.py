...
values (MRV) combined with least constrained value (LCV) in addition to Forward 
Checking (FC). Rather than working through the chessboard row by row, MRV will 
prioritize rows with minimum remaining safe spots at each step. Furthermore, LCV 
will order the safe columns based on the number of remaining safe spots after 
hypothetical Queen placement in each of them (in an ascending order). Forward 
Checking keeps track of the number of Queen threats each spot faces and after 
every Queen placement, it checks to make sure that the remaining unoccupied rows 
all have at least one safe spot (0 threats). Otherwise, it backtracks. Combining 
both ordering heuristics allows stronger and quicker detection of failures and 
early backtracks. The Forward Checking does not necessary do any different than 
MRV in this case, so no changes (same as MRV + LCV). 
"""
class N_Queens_Solver_MRV_LCV_FC:
    """
    ...
    def visualize_step(self):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{self.n}-Queens (MRV + LCV + Forward Checking)")
        ax.invert_yaxis()
    ...
    """
    This function runs the recursive N-Queens Solver with MRV + LCV + FC. 
    """
    def solve(self):
        if self.vis:
