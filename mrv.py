...
values (MRV), or most constraining variable (MCV). Rather than working through 
the chessboard row by row, MRV will prioritize rows with minimum remaining safe
spots at each step. This approach encourages early detection of failures. 
"""


class N_Queens_Solver_MRV:
    """
    ...
    def visualize_step(self):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{self.n}-Queens (MRV)")
        ax.invert_yaxis()
    ...
    """
    This function runs the recursive N-Queens Solver with backtracking and MRV. 
    """
